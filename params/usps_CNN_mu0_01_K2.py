...
def options():
    ...
    options = {'seed': 1226, 'tol': 0.001, 'mu': 0.1, 'K': 2}
    ...
    # Load the dataset
    options['train_loader'], options['valid_loader'] = get_train_valid_loader(batch_size=batch_size, augment=False)
    ...
    options['optimizer'] = torch.optim.Adam(options['model'].parameters())
    options['scheduler'] = torch.optim.lr_scheduler.LambdaLR(options['optimizer'], lr_lambda=alpha)
